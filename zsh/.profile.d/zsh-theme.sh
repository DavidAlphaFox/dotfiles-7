...
__git_ps1_show_upstream ()
{
	...
	if [[ -z "$verbose" ]]; then
		...
		*)	    # diverged from upstream
		...
		case "$count" in
		"") # no upstream
			p="%{$fg[yellow]%}$noup_sym%{$reset_color%}" ;;
		"0	0") # equal to upstream
			p="" ;;
		"0	"*) # ahead of upstream
			p="%{$fg[green]%}${count#0	}$up_sym%{$reset_color%}" ;;
		*"	0") # behind upstream
...
__git_ps1 ()
{
	...
	if [ "true" = "$inside_gitdir" ]; then
		...
		then
			...
			git diff --no-ext-diff --cached --quiet || i="%{$fg[yellow]%}$added_sym%{$reset_color%}"
	...
	b="%{$fg[yellow]%}${b##refs/heads/}%{$reset_color%}"
	if [ $pcmode = yes ] && [ $ps1_expanded = yes ]; then
...
local ret_status='%(?:%{$fg[$primary]%}$sh_sym:%{$fg[red]%}$err_sym%s)%{$reset_color%}'
local vicmd_status='%{$reset_color%}%{$fg[green]%}$vicmd_sym%{$reset_color%} '
local git_status='$(__promptline_vcs_branch)%{$reset_color%}'
local dir_status='%{$fg[$primary]%}$(get_pwd)%{$reset_color%}'
local host_status='%{$fg[yellow]%}$(__promptline_host)%{$reset_color%}'
