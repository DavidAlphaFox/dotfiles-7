...
__git_ps1_colorize_gitstring ()
{
	if [[ -n ${ZSH_VERSION-} ]]; then
		...
		local c_clear='\[\e[0m\]'
	        local c_orange='\[\e[1;31m\]'
	        local c_yellow='\[\e[1;33m\]'
		local c_lwhite='\[\e[1;37m\]'
	fi
	local bad_color=$c_red
	local ok_color=$c_green
	local flags_color="$c_orange"
...
__git_ps1 ()
{
	...
	local f="$u$w$i$s"
	local gitstring="$c$b${f:+$z$f}$r$p\[\e[0m\]"
...
function __default_ps1() {
    # $
    STAT="$?"
    if [ $STAT -eq 0 ] || [ -n "$WAIT_ONE" ]; then
        S="\[\033[${P_COLOR}m\]${S_SYM}\[\e[m\]"
	T="${PWD/$HOME/\~}"
        [ $STAT -eq 0 ] && unset WAIT_ONE
    else
        S="\[\033[31m\]${S_SYM}\[\e[m\]"
	T="exit status: $STAT"
    fi
